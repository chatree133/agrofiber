...
export default function QtDocumentTemplate({ docData }) {
      ...
      <div className="flex justify-between items-start border-b-2 border-indigo-600 pb-4 mb-6">
        ...
        <div className="text-right">
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] text-left mt-2 border border-slate-200 p-2 rounded bg-slate-50">
            ...
            {docData.ValidUntil && (
              <>
                <span className="font-semibold text-slate-600">ยืนยันราคาถึง:</span>
                <span className="text-slate-800">{dayjs(docData.ValidUntil).format('DD/MM/YYYY')}</span>
              </>
            )}
          </div>
        </div>
