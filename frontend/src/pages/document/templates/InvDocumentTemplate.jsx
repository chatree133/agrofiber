...
export default function InvDocumentTemplate({ docData }) {
  ...
  const dueDate = docData.DueDate || docData.dueDate;
  ...
  const billingAddress = docData.BillingAddress || docData.billingAddress || docData.ShippingAddress || docData.shippingAddress || '-';
  const soNo = docData.SalesOrderNo || docData.salesOrderNo || '-';
  const doNo = docData.DeliveryOrderNo || docData.deliveryOrderNo || '-';
  ...
  return (
    <div className="print-page pb-8">
      ...
      <div className="flex justify-between items-start border-b-2 border-indigo-600 pb-4 mb-6">
        ...
        <div className="text-right">
          <h2 className="text-base font-bold text-indigo-900 mb-1">
            ใบกำกับภาษี / ใบแจ้งหนี้<br />(TAX INVOICE / INVOICE)
          </h2>
          <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] text-left mt-2 border border-slate-200 p-2 rounded bg-slate-50">
            ...
            <span className="font-semibold text-slate-600">ครบกำหนด / Due Date:</span>
            <span className="text-red-600 font-medium">{dueDate ? dayjs(dueDate).format('DD/MM/YYYY') : '-'}</span>

            <span className="font-semibold text-slate-600">ใบสั่งขาย / SO No:</span>
            <span className="text-slate-800">{soNo}</span>

            <span className="font-semibold text-slate-600">ใบส่งสินค้า / DO No:</span>
            <span className="text-slate-800">{doNo}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        <div className="border border-slate-200 rounded p-3 bg-slate-50 min-h-[95px]">
        ...
        <div className="border border-slate-200 rounded p-3 bg-slate-50 min-h-[95px]">
          <h3 className="font-bold text-indigo-900 border-b border-slate-200 pb-1 mb-2 text-[11px]">
            ที่อยู่ลูกค้าสำหรับเรียกเก็บเงิน (BILLING ADDRESS)
          </h3>
          <p className="text-slate-700 leading-relaxed whitespace-pre-wrap">{billingAddress}</p>
        </div>
      </div>
      ...
      {/* Signature Section */}
      <div className="grid grid-cols-3 gap-6 text-center text-[10px] mt-12 pt-6 border-t border-slate-100">
        ...
        <div className="flex flex-col items-center justify-between min-h-[90px]">
          <div className="w-32 border-b border-slate-300 mb-2"></div>
          <span className="font-bold text-indigo-900">ผู้อนุมัติ (Authorized Approval)</span>
          <span className="text-slate-400">วันที่ / Date: ____/____/____</span>
        </div>
      </div>
    </div>
