...
import { Avatar, Button, Card, Divider, Input, Space, Table, Tag, Typography, message, Tooltip } from 'antd';
...
function deriveSortTime(row) {
  return toDate(row.actionAt) || toDate(row.createdAt) || toDate(row.completedAt) || new Date(0);
}
...
export default function WmsDashboard() {
  const { getWmsWaves, getWmsTasks, getWmsWaveDetail, getWmsTaskDetail } = useWms();
  ...
  const fetchTimeline = async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const waves = (wavesRes?.data || []).map((w) => ({
        ...
        id: w.id,
        code: w.waveNo,
        status: w.status === 'open' && w.actionByName ? 'in_progress' : w.status,
      ...
      const tasks = (putawayRes?.data || [])
        ...
        .map((t) => ({
          ...
          id: t.id,
          code: `Task #${t.id}`,
          status: t.status === 'open' && t.actionByName ? 'in_progress' : t.status,
          createdAt: t.createdAt,
          actionAt: t.actionAt,
          completedAt: t.completedAt,
          actionByName: t.actionByName,
          createdByName: null,
          summary: t.referenceType ? `${t.referenceType} (${t.referenceId ?? '-'})` : '-',
        }));

      const merged = [...waves, ...tasks]
        .sort((a, b) => deriveSortTime(b).getTime() - deriveSortTime(a).getTime())
        .slice(0, 150);

      setRows(merged);
      setLastUpdatedAt(new Date().toISOString());

      if (!selectedKey && merged.length) {
  ...
  useEffect(() => {
    ...
    if (!row && filteredRows.length) {
      const fallbackIndex = Math.min(cycleCursorRef.current, filteredRows.length - 1);
      setSelectedKey(filteredRows[fallbackIndex]?.key || filteredRows[0].key);
    }
  ...
  useEffect(() => {
    ...
    if (idx >= 0) {
      ...
      cycleCursorRef.current = Math.min(cycleCursorRef.current, Math.max(cycleRows.length - 1, 0));
    }
  }, [cycleRows]);
  ...
  useEffect(() => {
    ...
    if (bodyEl) {
      const list = cycleRowsRef.current || [];
      const nearEnd = list.length > 0 && cycleCursorRef.current >= list.length - 2;
  ...
  const columns = useMemo(() => ([
    {
      ...
      render: (_, r) => (
        <span className="inline-block w-3">
          {r.key === selectedKey ? <span className="wms-dashboard-dot" /> : null}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      key: 'code',
      width: 170,
      render: (text) => <Text className="wms-dashboard-text" strong>{text}</Text>,
    },
    {
      title: 'สถานะ',
      dataIndex: 'status',
    ...
    },
    {
      ...
      key: 'actionByName',
      width: 170,
      render: (text) => <Text className="wms-dashboard-text">{text || '-'}</Text>,
    },
    {
      title: 'เวลา',
      key: 'time',
  ...
  const derivedDetail = useMemo(() => {
    ...
    let headerCode = detail.type === 'wave' ? detailData.waveNo : `Task #${detailData.id}`;
    let status = detail.type === 'wave' ? detailData.status : detailData.status;
    let createdAt = detail.type === 'wave' ? detailData.createdAt : detailData.createdAt;
    let actionAt = detail.type === 'wave' ? detailData.actionAt : detailData.actionAt;
    let completedAt = detail.type === 'wave' ? detailData.completedAt : detailData.completedAt;
    let createdByName = detail.type === 'wave' ? detailData.createdByName : null;
    let actionByName = detail.type === 'wave' ? detailData.actionByName : detailData.actionByName;
    let actionByAvatarUrl = detail.type === 'wave' ? detailData.actionByAvatarUrl : detailData.actionByAvatarUrl;
    ...
    for (const l of lines) {
      const k = l.itemCode || '-';
      const prev = itemAgg.get(k) || { itemCode: k, qty: 0, uom: l.uom || '-' };
      prev.qty += Number(l.qty || 0);
      itemAgg.set(k, prev);
    }
    const items = Array.from(itemAgg.values()).sort((a, b) => a.itemCode.localeCompare(b.itemCode));

    const pickLocations = Array.from(
      new Map(
        lines
          .filter((l) => l.from)
          .map((l) => [`${l.from}|${l.pallet || ''}`, { location: l.from, pallet: l.pallet || '-' }])
      ).values()
    );
    const putLocations = Array.from(
      new Map(
        lines
          .filter((l) => l.to)
          .map((l) => [`${l.to}|${l.pallet || ''}`, { location: l.to, pallet: l.pallet || '-' }])
      ).values()
    );
      ...
      <div className="wms-dashboard-block">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <Tooltip title={h.actionByName ? `ผู้ดำเนินการ: ${h.actionByName}` : 'ยังไม่มีผู้ดำเนินการ'}>
              <Avatar
                size={44}
                src={h.actionByAvatarUrl || undefined}
                icon={!h.actionByAvatarUrl ? <UserOutlined /> : undefined}
                style={!h.actionByAvatarUrl ? { backgroundColor: '#334155', color: '#cbd5e1' } : undefined}
              />
            </Tooltip>
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                {typeTag(h.type === 'wave' ? 'wave' : h.type)}
                <Text className="wms-dashboard-text text-base" strong>{h.code}</Text>
              </div>
              <Text className="wms-dashboard-muted text-xs">
                สร้าง: {formatTime(h.createdAt)} | เริ่มทำ: {formatTime(h.actionAt)} | เสร็จ: {formatTime(h.completedAt)}
              </Text>
            </div>
          </div>
          <Tag className="wms-dashboard-status" color={statusColor(h.status)}>
            {h.status}
          </Tag>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {kv('จำนวนรวม', `${Number(derivedDetail.totals.totalQty || 0).toLocaleString('th-TH')} ${derivedDetail.items[0]?.uom || ''}`)}
        </div>
      </div>
    );
  }, [derivedDetail]);
  ...
  const itemsTable = useMemo(() => {
    if (!derivedDetail) return null;
    const cols = [
      { title: 'SKU', dataIndex: 'itemCode', key: 'itemCode', width: 160, render: (t) => <Text className="wms-dashboard-text" strong>{t}</Text> },
      { title: 'จำนวน', dataIndex: 'qty', key: 'qty', width: 120, align: 'right', render: (v) => <Text className="wms-dashboard-text">{Number(v || 0).toLocaleString('th-TH')}</Text> },
      { title: 'หน่วย', dataIndex: 'uom', key: 'uom', width: 90, render: (t) => <Text className="wms-dashboard-muted">{t || '-'}</Text> },
    ];
    return (
      <div className="wms-dashboard-block">
        <div className="flex items-center justify-between">
          <Text className="wms-dashboard-text" strong>รายการสินค้า</Text>
          <Text className="wms-dashboard-muted text-xs">รวม {derivedDetail.totals.skuCount} SKU / {derivedDetail.totals.lineCount} รายการ</Text>
        </div>
        <div className="mt-2">
          <Table
            size="small"
            columns={cols}
            dataSource={derivedDetail.items.map((it) => ({ ...it, key: it.itemCode }))}
            pagination={false}
  ...
  const pickPutBlocks = useMemo(() => {
    if (!derivedDetail) return null;
    const type = derivedDetail.header.type === 'wave' ? 'picking' : derivedDetail.header.type;

    const pickTitle = type === 'putaway' ? 'ตำแหน่งที่ต้องไปหยิบ (N/A)' : 'ตำแหน่งที่ต้องไปหยิบ';
    const putTitle = type === 'picking' ? 'ตำแหน่งที่ต้องไปวาง (N/A)' : 'ตำแหน่งที่ต้องไปวาง';

    const pickData = derivedDetail.pickLocations;
    const putData = derivedDetail.putLocations;

    const locCols = [
      { title: 'Location', dataIndex: 'location', key: 'location', width: 140, render: (t) => <Text className="wms-dashboard-text">{t || '-'}</Text> },
      { title: 'Pallet', dataIndex: 'pallet', key: 'pallet', width: 160, render: (t) => <Text className="wms-dashboard-muted">{t || '-'}</Text> },
    ];

    return (
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        <div className={`wms-dashboard-block ${type === 'putaway' ? 'opacity-50' : ''}`}>
          <Text className="wms-dashboard-text" strong>{pickTitle}</Text>
          <div className="mt-2">
            <Table
              size="small"
              columns={locCols}
              dataSource={(pickData || []).slice(0, 10).map((d, idx) => ({ key: idx, ...d }))}
              pagination={false}
              className="wms-dashboard-table"
            />
          </div>
        </div>
        <div className={`wms-dashboard-block ${type === 'picking' ? 'opacity-50' : ''}`}>
          <Text className="wms-dashboard-text" strong>{putTitle}</Text>
          <div className="mt-2">
            <Table
              size="small"
              columns={locCols}
              dataSource={(putData || []).slice(0, 10).map((d, idx) => ({ key: idx, ...d }))}
              pagination={false}
              className="wms-dashboard-table"
            />
          </div>
    ...
    <div className="wms-dashboard flex flex-col gap-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col">
          <h1 className="text-lg font-semibold wms-dashboard-text">WMS Dashboard</h1>
          <Text className="wms-dashboard-muted text-sm">งานคลังแบบ timeline + รายละเอียดแบบเรียลไทม์</Text>
          <Text className="wms-dashboard-muted text-xs">
            อัปเดตล่าสุด: {lastUpdatedAt ? new Date(lastUpdatedAt).toLocaleTimeString('th-TH') : '-'} {refreshing ? ' (กำลังรีเฟรช...)' : ''}
          </Text>
          <Text className="wms-dashboard-muted text-xs">
            Auto-play: {cycleMeta.total ? `${cycleMeta.idx + 1}/${cycleMeta.total}` : '-'}
          </Text>
        </div>
        <Space>
          <Input
          ...
          />
          <Button icon={<ReloadOutlined />} onClick={() => fetchTimeline({ silent: false })} loading={loading}>
            รีเฟรช
          </Button>
          <Button
            icon={autoRun ? <PauseOutlined /> : <PlayCircleOutlined />}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1.45fr_1fr]">
        ...
        >
          <div ref={timelineHostRef}>
            <Table
              ...
              scroll={{ x: 900, y: 560 }}
              className="wms-dashboard-table"
              rowClassName={(record) => (record.key === selectedKey ? 'wms-dashboard-row-active' : '')}
              onRow={(record) => ({
                onClick: () => {
                  const idx = (cycleRowsRef.current || []).findIndex((r) => r.key === record.key);
                  if (idx >= 0) cycleCursorRef.current = idx;
        ...
        <Card title="Task Detail" className="wms-dashboard-card shadow-sm">
