...
export default function GoodsReceiptList() {
    ...
    const { getGoodsReceipts, getGoodsReceiptTypes } = useGoodsReceipt();
    ...
    const fetchGoodsReceipts = async () => {
        ...
        try {
            const params = {
                ...
                goodsReceiptTypeId: typeId || undefined,
            ...
            message.error("โหลดข้อมูลใบรับสินค้าล้มเหลว: " + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getStatusTag = (status) => {
        const statuses = {
            ...
            received: { label: "รับสินค้าแล้ว", color: "blue" },
            posted: { label: "โพสต์เข้าคลังแล้ว", color: "success" },
    ...
    const columns = [
        {
            ...
            key: "documentNo",
            render: (text, record) => (
                <Button type="link" onClick={() => navigate(`/inventory/goods-receipts/${record.id}`)} style={{ padding: 0 }}>
                    {text}
                </Button>
            ),
        },
        {
            ...
            render: (text, record) => (
                <Tag color="cyan">{text || record.goodsReceiptTypeCode}</Tag>
            ),
        },
        {
            title: "คลังสินค้า",
        ...
        {
            title: "ผู้จัดจำหน่าย (Vendor)",
            dataIndex: "vendorName",
            key: "vendorName",
            render: (text, record) => record.vendorCode ? `[${record.vendorCode}] ${text}` : "-",
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Button
                        type="link"
                        icon={<EyeOutlined />}
                        onClick={() => navigate(`/inventory/goods-receipts/${record.id}`)}
                    >
                        ดูรายละเอียด
                    </Button>
                    {record.status === "draft" && (
                        <Button
                            type="link"
                            icon={<EditOutlined />}
                            onClick={() => navigate(`/inventory/goods-receipts/${record.id}/edit`)}
                        >
                            แก้ไข
                        </Button>
                    )}
            ...
            <div className="flex justify-between items-center mb-6" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                <div>
                    ...
                    <Paragraph type="secondary" style={{ marginBottom: 0 }}>
                        การรับสินค้าเข้าคลังแมนนวล เช่น รับคืนจากการเคลม รับสินค้าตัวอย่าง หรือรับวัตถุดิบอื่น ๆ
                    </Paragraph>
                </div>
                <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate("/inventory/goods-receipts/create")}>
                    สร้างใบรับสินค้า
                </Button>
            </div>
            ...
            <Card style={{ marginBottom: "20px", background: "#fafafa" }} bodyStyle={{ padding: "16px" }}>
                <Row gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={6}>
                        ...
                        <Input.Search
                            placeholder="ค้นหา เลขที่เอกสาร, ผู้จัดจำหน่าย..."
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            onSearch={fetchGoodsReceipts}
                            allowClear
                        />
                    </Col>
                    <Col xs={24} sm={12} md={6}>
                    ...
                    <Col xs={24} sm={12} md={6}>
                        ...
                        >
                            ...
                            <Select.Option value="received">รับสินค้าแล้ว (Received)</Select.Option>
                            <Select.Option value="posted">โพสต์เข้าคลังแล้ว (Posted)</Select.Option>
