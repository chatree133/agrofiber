...
import { Card, Table, Tag, Input, Select, Checkbox, Row, Col, Statistic, Typography, Button, Space, Modal, Tooltip, Empty } from 'antd';
import {
  DatabaseOutlined,
  HomeOutlined,
  EnvironmentOutlined,
...
export default function StockList() {
  ...
  // Statistic Totals
  const { totalPhysical, totalReserved, totalAvailable, totalUniqueItems } = useMemo(() => {
    let physical = 0;
    let reserved = 0;
    let available = 0;
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {record.salesSku && (
            <Tag color="blue" style={{ marginTop: '4px', width: 'fit-content', fontSize: '11px' }}>
              SKU: {record.salesSku}
            </Tag>
          )}
        </div>
      ...
      render: (_, record) => {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', fontSize: '13px' }}>
            {hasSpec ? (
              <>
                <Text>{record.specName || record.specCode}</Text>
                {record.specCode && <Text type="secondary" style={{ fontSize: '11px' }}>โค้ด: {record.specCode}</Text>}
              </>
            ) : (
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <Space size={4}>
            ...
            <Text strong style={{ fontSize: '13px' }}>{record.warehouseCode}</Text>
            <Text type="secondary" style={{ fontSize: '12px' }}>{record.warehouseName}</Text>
          </Space>
          {record.locationCode && (
            <Tag color="cyan" style={{ width: 'fit-content', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <EnvironmentOutlined />
              {record.locationCode}
            </Tag>
          )}
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {record.lotNo ? (
            <Tag color="purple" style={{ width: 'fit-content', fontWeight: 'bold' }}>
              LOT: {record.lotNo}
            </Tag>
          ) : (
            <Text type="secondary">-</Text>
          ...
          {record.gradeName && (
            <Text style={{ fontSize: '12px', color: '#595959' }}>เกรด: {record.gradeName}</Text>
          )}
        </div>
      )
    },
    {
      ...
      render: (val) => <Text strong style={{ fontSize: '14px', color: '#1890ff' }}>{val.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</Text>
    },
    {
      ...
      render: (val) => (
        <Text style={{ fontSize: '14px', color: val > 0 ? '#fa8c16' : '#bfbfbf' }}>
          {val > 0 ? val.toLocaleString('th-TH', { minimumFractionDigits: 2 }) : '0.00'}
        </Text>
      )
    },
    {
      ...
      render: (val) => (
        <Text strong style={{ fontSize: '15px', color: val > 0 ? '#52c41a' : '#fa541c' }}>
  ...
  const movementColumns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space direction="vertical" size={0}>
          <Text strong style={{ fontSize: '12px' }}>{record.referenceType || 'N/A'}</Text>
          <Text type="secondary" style={{ fontSize: '11px' }}>ID: #{record.referenceId}</Text>
        </Space>
      )
    },
    {
      ...
      key: 'fromLocation',
      render: (_, record) => {
        if (!record.fromWarehouseCode) return <Text type="secondary">-</Text>;
        return (
          <div>
            <Text>{record.fromWarehouseCode}</Text>
            {record.fromLocationCode && <Text type="secondary" style={{ fontSize: '11px', display: 'block' }}>({record.fromLocationCode})</Text>}
          </div>
        );
      }
    },
    {
      ...
      key: 'toLocation',
      render: (_, record) => {
        if (!record.toWarehouseCode) return <Text type="secondary">-</Text>;
        return (
          <div>
            <Text>{record.toWarehouseCode}</Text>
            {record.toLocationCode && <Text type="secondary" style={{ fontSize: '11px', display: 'block' }}>({record.toLocationCode})</Text>}
          </div>
        );
      }
    },
  ...
  return (
    <div style={{ padding: '4px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      {/* Header Panel */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <Text type="secondary">ตรวจสอบและสืบค้นยอดคงเหลือ รายคลัง รายตำแหน่งจัดเก็บ และเลขล็อต แบบ Real-time</Text>
        </div>
        <Space>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} md={6}>
          <Card
            ...
            style={{
              borderRadius: '12px',
              background: 'linear-gradient(135deg, #eef7f2 0%, #d8eede 100%)',
              boxShadow: '0 4px 20px rgba(11, 115, 62, 0.05)'
            }}
          >
            <Statistic
              title={<Text strong style={{ color: '#0b733e' }}>รายการสินค้าในสต็อก</Text>}
              value={totalUniqueItems}
              valueStyle={{ color: '#071d2c', fontSize: '26px', fontWeight: 'bold' }}
              suffix={<span style={{ fontSize: '13px', color: '#595959' }}> SKU</span>}
              prefix={<PieChartOutlined style={{ color: '#0b733e', marginRight: '6px' }} />}
            />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card
            ...
            style={{
              borderRadius: '12px',
              background: 'linear-gradient(135deg, #ebf3ff 0%, #d0e3ff 100%)',
              boxShadow: '0 4px 20px rgba(24, 144, 255, 0.05)'
            }}
          >
            <Statistic
              title={<Text strong style={{ color: '#1890ff' }}>คงคลังกายภาพรวม</Text>}
              value={totalPhysical}
              precision={2}
              valueStyle={{ color: '#071d2c', fontSize: '26px', fontWeight: 'bold' }}
              suffix={<span style={{ fontSize: '13px', color: '#595959' }}> แผ่น</span>}
            />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <Statistic
              title={<Text strong style={{ color: '#fa8c16' }}>ยอดจองสินค้า</Text>}
              value={totalReserved}
              precision={2}
              valueStyle={{ color: '#fa8c16', fontSize: '26px', fontWeight: 'bold' }}
              suffix={<span style={{ fontSize: '13px', color: '#fa8c16' }}> แผ่น</span>}
            />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card
            ...
            style={{
              borderRadius: '12px',
              background: 'linear-gradient(135deg, #eef9eb 0%, #dbf3d4 100%)',
              boxShadow: '0 4px 20px rgba(82, 196, 26, 0.05)'
            }}
          >
            <Statistic
              title={<Text strong style={{ color: '#52c41a' }}>พร้อมขายรวม</Text>}
              value={totalAvailable}
              precision={2}
              valueStyle={{ color: '#52c41a', fontSize: '26px', fontWeight: 'bold' }}
              suffix={<span style={{ fontSize: '13px', color: '#52c41a' }}> แผ่น</span>}
            />
          </Card>
        </Col>
      </Row>
      ...
      >
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col xs={12} md={4} style={{ textAlign: 'right' }}>
            <Space>
              <Button type="primary" style={{ backgroundColor: '#0b733e', borderColor: '#0b733e' }} onClick={fetchStock}>
                ค้นหา
              </Button>
      ...
      >
        <Table
          ...
          locale={{
            emptyText: <Empty description="ไม่พบข้อมูลสินค้าคงเหลือในระบบคลังสินค้า" />
          }}
      ...
      <Modal
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <HistoryOutlined style={{ color: '#0b733e', fontSize: '18px' }} />
            <span>ประวัติการเคลื่อนไหวสต็อก (Stock Movement History)</span>
          </div>
        }
        ...
        footer={[
          <Button key="close" onClick={() => setMovementModalOpen(false)}>
            ปิดหน้าต่าง
          </Button>
        ]}
        width={950}
      >
        {selectedStockRow && (
          <div style={{ marginBottom: '16px', background: '#f5f8fb', padding: '12px 16px', borderRadius: '8px' }}>
            <Row gutter={[16, 8]}>
              <Col xs={24} sm={12}>
                <Space direction="vertical" size={2}>
                  <Text type="secondary" style={{ fontSize: '12px' }}>ข้อมูลสินค้า</Text>
                  <Text strong style={{ color: '#071d2c' }}>
                    {selectedStockRow.itemCode} - {selectedStockRow.itemName}
                  </Text>
                  {selectedStockRow.salesSku && (
                    <Text type="secondary" style={{ fontSize: '12px' }}>SKU: {selectedStockRow.salesSku}</Text>
                  )}
                </Space>
              </Col>
              <Col xs={24} sm={12}>
                <Row gutter={16}>
                  <Col span={8}>
                    <Statistic
                      title={<span style={{ fontSize: '12px', color: '#8c8c8c' }}>คงคลังกายภาพ</span>}
                  ...
                  <Col span={8}>
                    <Statistic
                      title={<span style={{ fontSize: '12px', color: '#8c8c8c' }}>ยอดจอง</span>}
                      value={selectedStockRow.qtyReserved}
                      valueStyle={{ fontSize: '16px', fontWeight: 'bold', color: '#fa8c16' }}
                      precision={2}
                    />
                  </Col>
                  <Col span={8}>
                    <Statistic
                      title={<span style={{ fontSize: '12px', color: '#8c8c8c' }}>พร้อมขาย</span>}
                      value={selectedStockRow.qtyAvailable}
        ...
        <Table
          ...
          bordered
          locale={{
            emptyText: <Empty description="ไม่พบประวัติการเคลื่อนไหวสต็อกสำหรับสินค้านี้" />
          }}
        />
      </Modal>
    </div>
