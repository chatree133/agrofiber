...
// 6. Fix Keys
// UNIQUE KEY name (col) -> CONSTRAINT UQ_name UNIQUE (col)
sql = sql.replace(/UNIQUE KEY\s+(\w+)\s+\((.*?)\)/ig, 'CONSTRAINT UQ_$1 UNIQUE ($2)');
// KEY name (col) -> just remove it because inline INDEX is not supported in CREATE TABLE in older MSSQL, or it causes syntax errors.
...
while ((match = insertRegex.exec(sql)) !== null) {
    ...
    // We split by ")," but carefully. We assume ")," is between rows.
...
console.log('Conversion successful. Output saved to sqls/mssql_thailand-geodata.sql');
